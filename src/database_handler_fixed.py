...
class DatabaseHandler:
        ...
        while attempt < self._retry_attempts:
            ...
            try:
                ...
                if attempt < self._retry_attempts:
                    ...
                    log.warning("SQL attempt %d/%d failed for: %s. Retrying in %.1fs...",
                               attempt, self._retry_attempts, sql[:50], wait_time)
                    time.sleep(wait_time)
                else:
                    log.error("All SQL attempts failed for: %s", sql)
    ...
    def fetchall(self, sql: str, params: Optional[Iterable[Any]] = None) -> List[sqlite3.Row]:
        ...
        while attempt < self._retry_attempts:
            ...
            try:
                ...
                if attempt < self._retry_attempts:
                    wait_time = 0.1 * (2 ** (attempt - 1))
                    log.warning("fetchall attempt %d/%d failed. Retrying in %.1fs...",
                               attempt, self._retry_attempts, wait_time)
                    time.sleep(wait_time)
                else:
                    log.error("All fetchall attempts failed")
        
        if last_exc:
            log.exception("Database fetchall failed")
        return []
    ...
    def insert_or_replace(self, table: str, data: Dict[str, Any], commit: bool = True) -> bool:
        """Insert or replace single row"""
        try:
